...
var resourceSchema = new mongoose.Schema({ name: String, url: String, cat: String, desc: String, access: String, created_at: String });
var accountingSchema = new mongoose.Schema({ title: String, amount: String, description: String, created_at: String });
var contractorSchema = new mongoose.Schema({ company: String, phone: String, service: String, docType: String, amount: String, created_at: String });
var hrSchema = new mongoose.Schema({ title: String, url: String, description: String, created_at: String });
var contactSchema = new mongoose.Schema({ name: String, position: String, phone: String, email: String, created_at: String });
var clickSchema = new mongoose.Schema({ userId: String, resourceId: String, userDept: String, timestamp: { type: Date, default: Date.now } });
...
var db = {
  ...
  async createUser(data) {
    ...
    return await new User({ name: data.name, email: data.email.toLowerCase().trim(), dept: data.dept, role: data.role, password: data.password, created_at: new Date().toLocaleString('uk-UA') }).save();
  },
  ...
  async getRecommendations(userId) {
    ...
    var personalFull = personal.map(function(p) { var r = resMap[p._id]; return r ? { resource: r, clicks: p.count } : null; }).filter(Boolean);
    var departmentFull = department.filter(function(d) { return personalIds.indexOf(d._id) === -1; }).map(function(d) { var r = resMap[d._id]; return r ? { resource: r, clicks: d.count } : null; }).filter(Boolean);
  ...
  async createTicket(data) {
    return await new Ticket({ title: data.title, cat: data.cat, priority: data.priority, desc: data.desc, status: 'new', author: data.author, authorId: data.authorId, dept: data.dept, assignedTo: data.assignedTo || '', assignedToName: data.assignedToName || 'Не призначено', createdAt: new Date().toLocaleString('uk-UA') }).save();
  },

  async updateTicketStatus(mongoId, status) { return await Ticket.findByIdAndUpdate(mongoId, { status: status }, { new: true }); },
  async updateTicketAssignee(mongoId, assignedTo, assignedToName) { return await Ticket.findByIdAndUpdate(mongoId, { assignedTo: assignedTo, assignedToName: assignedToName }, { new: true }); },
  ...
  async createAccounting(data) { return await new Accounting({ title: data.title, amount: data.amount, description: data.description || '', created_at: new Date().toLocaleString('uk-UA') }).save(); },
  async updateAccounting(mongoId, data) { return await Accounting.findByIdAndUpdate(mongoId, { title: data.title, amount: data.amount, description: data.description }, { new: true }); },
  ...
  async createContractor(data) { return await new Contractor({ company: data.company, phone: data.phone, service: data.service || '', docType: data.docType || 'general', amount: data.amount || '', created_at: new Date().toLocaleString('uk-UA') }).save(); },
  async updateContractor(mongoId, data) { return await Contractor.findByIdAndUpdate(mongoId, { company: data.company, phone: data.phone, service: data.service, docType: data.docType || 'general', amount: data.amount || '' }, { new: true }); },
  ...
  async createHR(data) { return await new HR({ title: data.title, url: data.url, description: data.description || '', created_at: new Date().toLocaleString('uk-UA') }).save(); },
  async updateHR(mongoId, data) { return await HR.findByIdAndUpdate(mongoId, { title: data.title, url: data.url, description: data.description }, { new: true }); },
  async deleteHR(mongoId) { return await HR.findByIdAndDelete(mongoId); },

  async getContacts() { return await Contact.find().sort({ name: 1 }); },
  async createContact(data) { return await new Contact({ name: data.name, position: data.position, phone: data.phone, email: data.email, created_at: new Date().toLocaleString('uk-UA') }).save(); },
  ...
  async addLog(action, userName) { await new Log({ action: action, user_name: userName || 'Система', created_at: new Date().toLocaleString('uk-UA') }).save(); },

  async getStats() {
    var results = await Promise.all([User.countDocuments(), Resource.countDocuments(), Ticket.countDocuments(), User.countDocuments({ role: 'admin' }), User.countDocuments({ role: 'manager' }), Click.countDocuments()]);
    return { totalUsers: results[0], totalResources: results[1], totalTickets: results[2], adminCount: results[3], managerCount: results[4], totalClicks: results[5] };
  }
};
